...
void loop() {
  ...
  if(bit_ctr == 0) {
    ...
    sample_buffer[sample_ctr*SAMPLE_BYTE_LEN] = (current_sample >> 24) & 0xFF;
    sample_buffer[sample_ctr*SAMPLE_BYTE_LEN + 1] = (current_sample >> 16) & 0xFF;
    sample_buffer[sample_ctr*SAMPLE_BYTE_LEN + 2] = (current_sample >> 8) & 0xFF;
    
    /*int val_ = sample_buffer[sample_ctr] << 16 + 
                sample_buffer[sample_ctr + 1] << 8 + 
                sample_buffer[sample_ctr + 2];*/
    /*Serial.print(current_sample >> 24);
    Serial.print(" ");
    Serial.print(current_sample >> 16);
    Serial.print(" ");
    Serial.println(current_sample >> 8);*/
  ...
  if(sample_ctr == SAMPLE_BUFFER_LEN) {
    ...
    for(int i=0 ; i < SAMPLE_BUFFER_LEN ; i++) {
      int val_ = sample_buffer[i*SAMPLE_BYTE_LEN] << 16 + 
                  sample_buffer[i*SAMPLE_BYTE_LEN + 1] << 8 + 
                  sample_buffer[i*SAMPLE_BYTE_LEN + 2];
      Serial.println(val_);
    }
    sample_ctr = 0;
  }
